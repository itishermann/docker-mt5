...
function setup_root_crontab() {
  ...
  apt $APT_OPT install cron

  cron_line=$(bash -c "crontab -l 2>/dev/null" | grep -o "mkdir -p -m0755 /var/run/sshd")

  if [ -z "$cron_line" ]; then
    bash -c "cat <(crontab -l 2>/dev/null) <(echo '@reboot if [ ! -e /var/run/sshd  ]; then mkdir -p -m0755 /var/run/sshd; fi') | crontab"
  fi
...
function set_up_locale_and_timezone() {
  apt $APT_OPT install dbus

  # setting local and timezone.
  apt $APT_OPT install tzdata

  if [ "$ID" == "debian" ]; then
    # for debian
    apt $APT_OPT install task-japanese locales
  else
    apt $APT_OPT install language-pack-ja
  fi

  # 1 locale only
  bash -c 'echo "'$TARGET_LOCALE' UTF-8" > /etc/locale.gen'
...
function install_packages_misc_and_needed_by_mt4() {
  # install misc
  apt $APT_OPT update
  apt $APT_OPT install unattended-upgrades apt-transport-https ntp psmisc file bc vim nano less tmux curl net-tools lsof

  # install gui
  # apt $APT_OPT install tightvncserver fonts-vlgothic xterm wm2

  # install wine
  apt $APT_OPT install software-properties-common
  ...
  if [ -z "$REPOS_EXISTS" ]; then
    ...
    apt-key add "$DIR_WINECACHE/winehq.key"
  ...
  if [ -z "$REPOS_EXISTS" ]; then
    ...
    if [ -n "$WEB_REPOS_EXITS" ]; then
      ...
      apt-key add "$DIR_WINECACHE/Release.key"
      rm "$DIR_WINECACHE/Release.key"
      apt-add-repository "deb $FAUDIO_REPOS ./"
    fi
  fi

  apt $APT_OPT update
  apt $APT_OPT install --install-recommends winehq-stable
}
...
install_packages_needed_by_graphical_report() {
  apt $APT_OPT install gnuplot-nox vncsnapshot
}
...
function setup_auto_update_package_list() {
  ...
  if [ ! -e "$upgrades_conf" ]; then
    cat - << EOS | bash -c "cat - > '$upgrades_conf'"
...
EOS
    ...
    if [ -z "$(grep -io 'APT::Periodic::Update-Package-Lists' "$upgrades_conf")" ]; then
      echo 'APT::Periodic::Update-Package-Lists "1";' | bash -c 'cat - >> "'$upgrades_conf'"'
    else
      sed -i -e 's/APT::Periodic::Update-Package-Lists "[0-9][0-9]*"/APT::Periodic::Update-Package-Lists "1"/g'  "$upgrades_conf"
    fi

    # Unattended-Upgrade
    if [ -z "$(grep -io 'APT::Periodic::Unattended-Upgrade' "$upgrades_conf")" ]; then
      echo 'APT::Periodic::Unattended-Upgrade "0";' | bash -c 'cat - >> "'$upgrades_conf'"'
    else
      sed -i -e 's/APT::Periodic::Unattended-Upgrade "[0-9][0-9]*"/APT::Periodic::Unattended-Upgrade "0"/g'  "$upgrades_conf"
    fi
    # AutocleanInterval
    if [ -z "$(grep -io 'APT::Periodic::AutocleanInterval' "$upgrades_conf")" ]; then
      echo 'APT::Periodic::AutocleanInterval "1";' | bash -c 'cat - >> "'$upgrades_conf'"'
    else
      sed -i -e 's/APT::Periodic::AutocleanInterval "[0-9][0-9]*"/APT::Periodic::AutocleanInterval "1"/g'  "$upgrades_conf"
    fi
  fi
  ...
  if [ -n "$is_apt_daily_timer_exists" ]; then
    ...
    if [ ! -f /etc/cron.daily/apt-compat ]; then
      mv /etc/cron.daily/apt-compat.disabled /etc/cron.daily/apt-compat
    fi
...
function setup_vncserver {
  if [ -n "$IS_SYSTEMD" ]; then
    ...
    if [ ! -f "/etc/systemd/system/vncserver@:1.service" ]; then
      install -o root -g root -m 644 -D "$DIR_TEMPLATES/vncserver@:1.service" "/etc/systemd/system/vncserver@:1.service"
      sed -i -e 's/%%USER_NAME%%/'$ORG_USER'/g' "/etc/systemd/system/vncserver@:1.service"
    fi
    ...
    if [ ! -f "/etc/init.d/vncserver" ]; then
      install -o root -g root -m 644 -D "$DIR_TEMPLATES/vncserver" "/etc/init.d/vncserver"
      sed -i -e 's/%%USER_NAME%%/'$ORG_USER'/g' "/etc/init.d/vncserver"
